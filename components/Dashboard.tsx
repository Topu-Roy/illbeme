...
export function Dashboard({ onStartReflection }: DashboardProps) {
  const { data: entries } = api.journal.getJournals.useQuery({ date: new Date() });
  ...
  const todayEntries = entries?.filter((e) => new Date(e.createdAt).toDateString() === new Date().toDateString());
  ...
  return (
    <div className="mb-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">{todayCheckIn?.overallMood ?? lastCheckIn?.overallMood ?? "—"}</div>
...
export function EncouragementDialog() {
  const { data: encouragement, isLoading } = api.encouragement.getEncouragement.useQuery();
