              ...
              {emotionEntries.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {emotionEntries.map((emotion) => (
                    <Badge key={emotion.id} variant="secondary" className="px-3 py-1 text-sm">
                      {emotion.emotion} <span className="ml-2 opacity-70">x{emotion.count}</span>
                    </Badge>
                  ))}
                </div>
              ) : (
            ...
            <Card className="h-full">
              ...
              <CardContent>
                <p className="leading-relaxed whitespace-pre-wrap">
                  {checkIn.learnings.length < 1 ? (
                    <span className="text-muted-foreground italic">No entry.</span>
                  ) : (
                    checkIn.learnings.map((item) => <span key={item.id}>{item.content}</span>)
                  )}
                </p>
              </CardContent>
            </Card>
